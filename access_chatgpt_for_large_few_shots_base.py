...
df = pd.read_pickle('blueprint-primer-embeddings-with-tokens-mac.pkl')

#print(df.head(3))
#print(type(df['prompt_embedding'][0]))
#task = '''Learn about blueprint: '''

#query ="Who wrote the book The myth of Sisyphus?"
...
response = openai.ChatCompletion.create(model=model, messages = actual_prompt, temperature=0., max_tokens=max_tokens)
...
print('The GPT 3 engine responds with: ', response['choices'][0]['message']) 
